...
import com.reporting.metier.interfaces.RoamingInRemote;

@Stateless
public class RoamingINImpl implements RoamingInRemote {
	...
	public Map<Object, Number> getConfiguartionChartByCfg(String x, String y,
		...
		if(x.equals("dateAppel")){
			...
			Query q = em.createQuery("Select s.destination.typeDest," +action+" "+y+" " +"From StatTraficIn s  where typeCdr='roa'  Group By s.destination.typeDest");
			...
			Query q = em.createQuery("Select "+x+" ,"+action+" "+y+" " +"From StatTraficIn s where typeCdr='roa' "+" "+groupby);
			List<Object[]> resultList = q.getResultList();
			Map<Object, Number> resultMap = new HashMap<Object, Number>(resultList.size());
			for (Object[] result : resultList){
	...
	public Map<String, Number> getConfiguartionChartByCnfg(String x, String y,
		...
		if(x.equals("dateAppel")){
			...
			Query q = em.createQuery("Select to_date("+x+",'YYMMDD') ,"+action+" "+y+" " +"From StatTraficIn s  where typeCdr='roa' "+" "+groupby+" Order BY to_date(s.dateAppel,'YYMMDD') desc ");
			List<Object[]> resultList = q.getResultList().subList(0, 7);
	...
	public Map<Object, Number> getMscBytranche(String x, String y,
		...
		Query q = em.createQuery("Select CAST(trancheHoraire AS integer) ,"+action+" "+y+" " +"From StatTraficIn s"+" Where typeCdr='roa' AND "+where+" "+"Group By CAST(trancheHoraire AS integer) Order by  CAST(trancheHoraire AS integer) DESC ");
		List<Object[]> resultList = q.getResultList();
		Map<Object, Number> resultMap = new HashMap<Object, Number>(resultList.size());
		for (Object[] result : resultList){
	...
	public Map<Object, Number> getMscByFilters(String x, String y,
		...
		if(x.equals("dateAppel")){
			...
			Query q = em.createQuery("Select"+x+","+action+" "+y+" " +"From StatTraficIn s"+" Where typeCdr='roa' AND "+where+" "+groupby+" Order BY "+x+" DESC ");
			List<Object[]> resultList = q.getResultList();
	...
	public List<Object[]> getAllStatLocalIn(String x, List<String> list_y,
		...
		return em.createQuery("Select "+x+","+y+"From StatTraficIn s Where typeCdr='roa' AND  "+where+" Group By "+x+" Order By "+x ).getResultList();
	...
	public Map<String, Number> getMscByFilters1(String x, String y,
		...
		if(x.equals("s.destination.typeDest")|| x.equals("msc")){
			...
			Query q = em.createQuery("Select "+x+"," +action+" "+y+" " +"From StatTraficIn s"+" Where typeCdr='roa' AND "+where+" "+groupby);
			List<Integer> lst = em.createQuery("Select distinct t.id from TypeDestination t").getResultList();
			q.setParameter("List", lst);
			List<String> lst1 = em.createQuery("Select distinct t.codePlanTarifaire from PlanTarifaire t").getResultList();
			q.setParameter("List1", lst1);
			List<Object[]> resultList = q.getResultList();
			if(resultList.isEmpty()==false){
				Map<String, Number> resultMap = new HashMap<String, Number>(resultList.size());
				for (Object[] result : resultList){
	...
	public List<Object[]> getAllStatTrafficStatic(String x, String y,
		...
		if(x.contains("substring")){
			...
			List<Integer> lst = em.createQuery("Select distinct t.id from TypeDestination t").getResultList();
			q.setParameter("List", lst);
			List<String> lst1 = em.createQuery("Select distinct p.codePlanTarifaire from PlanTarifaire p").getResultList();
			q.setParameter("List1", lst1);
			return q.getResultList();
