...
public class StatReconMscInimpl implements StatReconRemote {
	...
	@Override
	public Map<Object, Number> getAllStatRecon(String x, String y,
	...
	public List<Object[]> getDetailsDestinationStatReconMscIn(String x,
			String where) {
		// TODO Auto-generated method stub
		return em.createQuery("Select "+x+", s.destination.typeDest ,SUM(nbAnomalie),SUM(dureeAnomalie),SUM(nbTotalMsc),SUM(dureeTotalMsc),SUM(nbTotalIn),SUM(dureeTotalIn),SUM(kpiNb),SUM(kpiDuree),SUM(impact) From StatDestinationReconMscIn s where "+where+" Group BY  s.destination.typeDest, "+x ).getResultList();
	}
