...
@ManagedBean(name="arpmRoaming")
public class ARPMRoaming {
	...
	private List<Object[]> resultarpmRoaming;

	
	public List<Object[]> getResultarpmRoaming() {
		return resultarpmRoaming;
	}
	public void setResultarpmRoaming(List<Object[]> resultarpmRoaming) {
		this.resultarpmRoaming = resultarpmRoaming;
	}
	...
	public void handlechange(AjaxBehaviorEvent event){
		...
		if(compJourOutputHeure!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				ValueExpression valjour = createValueExpression("#{arpmRoaming.date_ParJourDeb}", Date.class);
				...
				ValueExpression valjourfin = createValueExpression("#{arpmRoaming.date_ParJourFin}", Date.class);
			...
			ValueExpression valjour = createValueExpression("#{arpmRoaming.date_Parheure}", Date.class);
			cld_jour.setValueExpression("value", valjour);
				...
				lstDateYears.setId("lstDateYears");
				 ValueExpression valueExp = createValueExpression("#{arpmRoaming.date_year_deb}", Integer.class);
				...
				lstDateYearsFin.setId("lstDateYearsFin");
				 ValueExpression valueExp1 = createValueExpression("#{arpmRoaming.date_year_fin}", Integer.class);
			...
			comp.getChildren().add(lstDateYearsFin);
				...
				lstDateYearsMois.setId("lstDateYearsMois");
				 ValueExpression valueExp = createValueExpression("#{arpmRoaming.date_year}", Integer.class);
		         UISelectItems list_items0 = new UISelectItems();
				...
				lstMois.setId("lstMois");
				 ValueExpression valueExp1 = createValueExpression("#{arpmRoaming.date_mois_debut}", Integer.class);
		         UISelectItems list_items = new UISelectItems();
				...
				lstMoisFin.setId("lstMoisFin");
				 ValueExpression valueExp2 = createValueExpression("#{arpmRoaming.date_mois_Fin}", Integer.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			if(this.getPlan()!="" && this.getPlan()!=null){
				...
				SubTitle= SubTitle+" avec PlanTarifaire égale à  "+this.getPlan();
			}
		 ...
		 if(this.getChoix_periode().equals("Par Heure")){
			...
			// sc1.setMap(statINLoc.getMscBytranche("CAST(trancheHoraire AS integer)", "SUM(revenue)*60/SUM(duree)", "", "", where_liste));
			 resultarpmRoaming= statINLoc.getAllStatIN("CAST(trancheHoraire AS integer)",where_liste);
	 		 }else if(this.getChoix_periode().equals("Par An")){
	 ...
	 resultarpmRoaming= statINLoc.getAllStatIN("Extract (year from to_date(dateAppel,'YYMMDD'))",where_liste);
		 		 }else if(this.getChoix_periode().equals("Par Mois")){
			// sc1.setMap(statINLoc.getMscByFilters("Extract (month from to_date(dateAppel,'YYMMDD'))", "SUM(revenue)*60/SUM(duree)", "", "Group By Extract (month from to_date(dateAppel,'YYMMDD')) ", where_liste));	
			 resultarpmRoaming= statINLoc.getAllStatIN("Extract (month from to_date(dateAppel,'YYMMDD'))",where_liste);
		...
		// sc1.setMap(statINLoc.getMscByFilters("extract(day from  to_date(dateAppel,'YYMMDD'))", "SUM(revenue)*60/SUM(duree)", "", "Group By  extract(day from  to_date(dateAppel,'YYMMDD'))", where_liste));	
		 resultarpmRoaming= statINLoc.getAllStatIN("Extract (day from to_date(dateAppel,'YYMMDD'))",where_liste);
