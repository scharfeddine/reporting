...
@ManagedBean(name="msc_sup_in")
@ViewScoped
public class RpCdrMscSupInMbean implements Serializable {
...
//		System.out.println(deb);
		if(this.getChoix_periode().equals("Par Jour")){
			...
			where=" anomalie='duree MSC supérieur duree IN' AND to_date(dateAppel,'YYMMDD') = to_date("+"'"+selectedRecon[0].toString()+"'"+",'yyyy-MM-dd') "+" And codeMsc='"+selectedPoint+"'";
	...
	Subtitle1 = "par tranche Horaire à "+selectedRecon[0].toString();
		}else if(this.getChoix_periode().equals("Par An")){
	
		yDetail=",SUM(dureeTotalMsc),SUM(kpiDuree)";
		
		
		 where =" anomalie='duree MSC supérieur duree IN' AND Extract(year from to_date(dateAppel,'YYMMDD')) = "+Integer.valueOf(selectedRecon[0].toString())+ " And Extract(month from to_date(dateAppel,'YYMMDD')) >= 01 And Extract(month from to_date(dateAppel,'YYMMDD')) <= 12"+" And codeMsc='"+selectedPoint+"'";
 ...
 where="anomalie='duree MSC supérieur duree IN' AND Extract(year from to_date(dateAppel,'YYMMDD')) ="+year+" And Extract(month from to_date(dateAppel,'YYMMDD')) ="+mois+" And codeMsc='"+selectedPoint+"'";
xDetail="Extract(day from to_date(dateAppel,'YYMMDD'))  ";
//resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconMscIn(xDetail, where);

Subtitle1="Par Jour en mois "+selectedRecon[0].toString();
	}
...
public void setSubTitle(String subTitle) {
	...
	public void handlechange(AjaxBehaviorEvent event){
		...
		if(compJourOutputHeure!=null){
			...
			if(this.getChoix_periode().equals("Par Jour")){
				...
				ValueExpression valjour = createValueExpression("#{msc_sup_in.date_ParJourDeb}", Date.class);
				...
				ValueExpression valjourfin = createValueExpression("#{msc_sup_in.date_ParJourFin}", Date.class);
			...
			ValueExpression valjour = createValueExpression("#{msc_sup_in.date_Parheure}", Date.class);
			cld_jour.setValueExpression("value", valjour);
				...
				lstDateYears.setId("lstDateYears");
				 ValueExpression valueExp = createValueExpression("#{msc_sup_in.date_year_deb}", Integer.class);
				...
				lstDateYearsFin.setId("lstDateYearsFin");
				 ValueExpression valueExp1 = createValueExpression("#{msc_sup_in.date_year_fin}", Integer.class);
			...
			comp.getChildren().add(lstDateYearsFin);
				...
				lstDateYearsMois.setId("lstDateYearsMois");
				 ValueExpression valueExp = createValueExpression("#{msc_sup_in.date_year}", Integer.class);
		         UISelectItems list_items0 = new UISelectItems();
				...
				lstMois.setId("lstMois");
				 ValueExpression valueExp1 = createValueExpression("#{msc_sup_in.date_mois_debut}", Integer.class);
		         UISelectItems list_items = new UISelectItems();
				...
				lstMoisFin.setId("lstMoisFin");
				 ValueExpression valueExp2 = createValueExpression("#{msc_sup_in.date_mois_Fin}", Integer.class);
	...
	public void Valider(){
		...
		if(this.getChoix_periode().equals(" ")){
			...
			where_liste.add("anomalie='duree MSC supérieur duree IN' ");
			if(this.getChoix_periode().equals("Par Jour")){
					...
					String where=" anomalie='duree MSC supérieur duree IN' AND to_date(dateAppel,'YYMMDD') Between to_date("+"'"+deb+"'"+",'yyyy-MM-dd') And to_date("+"'"+fin+"'"+",'yyyy-MM-dd')";
				...
				String where ="anomalie='duree MSC supérieur duree IN' AND Extract(year from to_date(dateAppel,'YYMMDD')) >= "+deb+" And Extract(year from to_date(dateAppel,'YYMMDD')) <= "+fin;
		...
		String where=" anomalie='duree MSC supérieur duree IN' AND Extract(year from to_date(dateAppel,'YYMMDD')) = "+year+"  And Extract(month from to_date(dateAppel,'YYMMDD')) >= "+deb+" And Extract(month from to_date(dateAppel,'YYMMDD')) <= "+fin+"";
...
//		System.out.println(deb);
		if(this.getChoix_periode().equals("Par Jour")){
			...
			where=" anomalie='duree MSC supérieur duree IN' AND to_date(dateAppel,'YYMMDD') = to_date("+"'"+selectedRecon[0].toString()+"'"+",'yyyy-MM-dd') ";
	...
	Subtitle1 = "par tranche Horaire à "+selectedRecon[0].toString();
		...
		yDetail=",SUM(dureeTotalMsc),SUM(kpiDuree)";
		 ...
		 where =" anomalie='duree MSC supérieur duree IN' AND Extract(year from to_date(dateAppel,'YYMMDD')) = "+Integer.valueOf(selectedRecon[0].toString())+ " And Extract(month from to_date(dateAppel,'YYMMDD')) >= 01 And Extract(month from to_date(dateAppel,'YYMMDD')) <= 12";
		xDetail="Extract(month from to_date(dateAppel,'YYMMDD')) ";
		resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconMscIn(xDetail, where);
		Subtitle1="Par Mois en "+selectedRecon[0].toString();
	}else if(this.getChoix_periode().equals("Par Mois")){
		...
		System.out.println(year+""+deb+""+fin);
		Integer mois = Integer.valueOf(selectedRecon[0].toString());
 where="anomalie='duree MSC supérieur duree IN' AND Extract(year from to_date(dateAppel,'YYMMDD')) ="+year+" And Extract(month from to_date(dateAppel,'YYMMDD')) ="+mois;
xDetail="Extract(day from to_date(dateAppel,'YYMMDD'))  ";
resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconMscIn(xDetail, where);
