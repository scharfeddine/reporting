...
//		System.out.println(deb);
 ...
 where="anomalie='cdrs in inexistant' AND Extract(year from to_date(dateAppel,'YYMMDD')) ="+year+" And Extract(month from to_date(dateAppel,'YYMMDD')) ="+mois+" And codeMsc='"+selectedPoint+"'";
...
for(int i=1;i<=31;i++){
		...
		if(compJourOutputHeure!=null){
			...
			comp.getChildren().add(lstDateYearsFin);
				...
				lstDateYearsMois.setId("lstDateYearsMois");
				 ValueExpression valueExp = createValueExpression("#{in_inexistant.date_year}", Integer.class);
		         UISelectItems list_items0 = new UISelectItems();
				...
				lstMois.setId("lstMois");
				 ValueExpression valueExp1 = createValueExpression("#{in_inexistant.date_mois_debut}", Integer.class);
		         UISelectItems list_items = new UISelectItems();
				...
				lstMoisFin.setId("lstMoisFin");
				 ValueExpression valueExp2 = createValueExpression("#{in_inexistant.date_mois_Fin}", Integer.class);
	...
	public void Valider(){
		...
		String where=" anomalie='cdrs in inexistant' AND Extract(year from to_date(dateAppel,'YYMMDD')) = "+year+"  And Extract(month from to_date(dateAppel,'YYMMDD')) >= "+deb+" And Extract(month from to_date(dateAppel,'YYMMDD')) <= "+fin+"";
		...
		//String kfdf=	(String) resultCDRInexistant.get(0)[0].t;
		 ...
		 if(this.getChoix_periode().equals("Par An")){
			 ...
			 List<Object[]>  result = statRecon.getAllStatReconCDRIN("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(dureeTotalIn),SUM(dureeTotalMsc),SUM(dureeAnomalie)", where_liste);
				
 			 System.out.println(result.size());
			 for(Object[] obj:result){
				 map.put(obj[0], (Number) obj[1]);
				// System.out.println(obj[0]+" "+obj[1]+" "+obj[2]+" "+obj[3]+" ");
				 map1.put(obj[0], (Number) obj[2]);
				 map2.put(obj[0], (Number) obj[3]);
			 }
			 sc1.setMap(map);
				sc2.setMap(map1);
				sc3.setMap(map2);
		 			 ...
		 			 List<Object[]>  result = statRecon.getAllStatReconCDRIN("Extract (month from to_date(dateAppel,'YYMMDD'))", "SUM(dureeTotalIn),SUM(dureeTotalMsc),SUM(dureeAnomalie)", where_liste);
						List<Object[]> res = ListUtils.union(result1, result);
		 			 System.out.println(result.size());
					 for(Object[] obj:res){
						 map.put(obj[0], (Number) obj[1]);
						// System.out.println(obj[0]+" "+obj[1]+" "+obj[2]+" "+obj[3]+" ");
...
//		System.out.println(deb);
	List<Object[]> result1 = new ArrayList<Object[]>();
		...
		System.out.println(year+""+deb+""+fin);
		Integer mois = Integer.valueOf(selectedRecon[0].toString());
 where="anomalie='cdrs in inexistant' AND Extract(year from to_date(dateAppel,'YYMMDD')) ="+year+" And Extract(month from to_date(dateAppel,'YYMMDD')) ="+mois;
xDetail="Extract(day from to_date(dateAppel,'YYMMDD'))  ";
yDetail = ",SUM(dureeTotalIn),SUM(kpiDuree)";
resultCDRDetailsInexistant = statRecon.getDetailsDestinationStatReconMscIn(xDetail, where);
Subtitle1="Par Jour en "+selectedRecon[0].toString();
