	...
	public void Valider(){
		...
		pie2=stat_remote.getMscByFilters1("msc", "duree)","StatMsc", "SUM(", "Group By msc ", where_liste);
		 if(this.getChoix_periode().equals("Par Heure")){
			 ...
			 staticListStatMsc = stat_remote.getAllStatMscStatic("substring(trancheHoraire,1,2)", "SUM(moyenneDuree),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);
	 			 ...
	 			 staticListStatMsc = stat_remote.getAllStatMscStatic("Extract (year from to_date(dateAppel,'YYMMDD'))", "SUM(moyenneDuree),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);
		 			 ...
		 			 staticListStatMsc = stat_remote.getAllStatMscStatic("Extract (month from to_date(dateAppel,'YYMMDD'))", "SUM(moyenneDuree),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);

		 		 }else if (this.getChoix_periode().equals("Par Jour")){
		 			 staticListStatMsc = stat_remote.getAllStatMscStatic("Extract (day from to_date(dateAppel,'YYMMDD'))", "SUM(moyenneDuree),SUM(duree),SUM(nbMoyenne),SUM(nbAppel)", "", "", where_liste);
